...
result_file_path = '../results/result-open-targets.csv'
associations_path = '../data/snp/associations-open-targets'
            ...
            if pattern.match(file):
                ...
                with open(file_path, 'r') as f:
                    ...
                    for row in reader:
                        ...
                        url= f'https://www.ebi.ac.uk/ols4/api/ontologies/efo/terms?obo_id={efo}'
                        response = requests.get(url)
                        if response.status_code == 200:
                            response = response.json()
                            trait_name = response['_embedded']['terms'][0]['label']
                        
                        matrix_data_path = os.path.join(matrixes_path, f'chrom-{chrom}', f'{main_rsid}.csv')
                        if os.path.exists(matrix_data_path):            
