    ...
    for snp in snps_file.iterrows():
        ...
        if snp_associations == None:
                snp_associations_formated.append({
                    ...
                    'CHR': snp_dict['CHR'],
                    'BP': snp_dict['BP'],
                    'BASE_SNP': snp_dict['BASE_SNP'] if 'BASE_SNP' in snps_file.columns else float('NaN'),
                    'LD_VALUE': snp_dict['LD_VALUE'] if 'LD_VALUE' in snps_file.columns else float('NaN')
                })
        else:
        ...
        results_file.dropna(how='all', axis=1, inplace=True)
